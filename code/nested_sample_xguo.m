...
%prior_e = @(e)     1 / (e_bounds(2) - e_bounds(1));%raylpdf(e, emean);%
prior_e = @(e)     1/(e_sig*sqrt(2*pi))*exp( - (e - e_mu).^2./(2*e_sig.^2));
...
prior_s = @(s)      normpdf(s, s_mu, s_sig);
...
%e_alive = sample_prior(N_planets, N_alive, e_bounds, 'uniform');%sample_prior(N_planets, N_alive, [emean emean] , 'rayl');
...
%     if N_accept > N_reject
%         sigma_V = min( sigma_V*exp(1/N_accept), sigcap*(V_bounds(2) - V_bounds(1)) );
%         sigma_K = min( sigma_K*exp(1/N_accept), sigcap*(K_bounds(2) - K_bounds(1)) );
%         sigma_w = min( sigma_w*exp(1/N_accept), sigcap*(w_bounds(2) - w_bounds(1)) );
%         sigma_e = min( sigma_e*exp(1/N_accept), sigcap*(e_bounds(2) - e_bounds(1)) );
%         sigma_P = min( sigma_P*exp(1/N_accept), sigcap*(P_bounds(2) - P_bounds(1)) );
%         sigma_chi = min( sigma_chi*exp(1/N_accept), sigcap*(chi_bounds(2) - chi_bounds(1)) );
%         sigma_s = min( sigma_s*exp(1/N_accept), sigcap*(s_bounds(2) - s_bounds(1)) );
%     else
%         sigma_V = min( sigma_V*exp(-1/N_reject), sigcap*(V_bounds(2) - V_bounds(1)) );
%         sigma_K = min( sigma_K*exp(-1/N_reject), sigcap*(K_bounds(2) - K_bounds(1)) );
%         sigma_w = min( sigma_w*exp(-1/N_reject), sigcap*(w_bounds(2) - w_bounds(1)) );
%         sigma_e = min( sigma_e*exp(-1/N_reject), sigcap*(e_bounds(2) - e_bounds(1)) );
%         sigma_P = min( sigma_P*exp(-1/N_reject), sigcap*(P_bounds(2) - P_bounds(1)) );
%         sigma_chi = min( sigma_chi*exp(-1/N_reject), sigcap*(chi_bounds(2) - chi_bounds(1)) );
%         sigma_s = min( sigma_s*exp(-1/N_reject), sigcap*(s_bounds(2) - s_bounds(1)) );
%     end
    

    if live_plot & mod(i, 400) == 0
...
% compute Bayesian evidence Z (trapezoid rule integration)
logZ = log(exp(L-max(L))*W) + max(L);
fprintf(['Evidence: logZ = ' num2str(logZ) '\n']);
